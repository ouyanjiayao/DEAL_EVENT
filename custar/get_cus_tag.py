...
class UpdataCustomerTag:
    ...
    def execute_to(self,limit):
        ...
        cursor.execute("select t1.yz_open_id from (select yz_open_id from customer_order GROUP BY yz_open_id) t1 "
                       ...
                       "on t1.yz_open_id = t2.yz_open_id where t2.syn_tag !=1 limit %s",(limit))
        ...
        if len(yz_id_list) > 0:
            ...
            self.conn.commit()   
        self.is_active(cursor,yz_id_list)
        self.leave_date(cursor, yz_id_list)
        self.sum_cost(cursor, yz_id_list)
        self.goods_like(cursor, yz_id_list)
        self.buy_once(cursor, yz_id_list)
        self.great_deal(cursor, yz_id_list)
        self.expand(cursor,yz_id_list)
        self.conn.close()


    # 购买是否积极
    def is_active(self,cursor,yz_id_list):
        yz_ids2 = []
        ...
        if len(yz_ids2) > 0:
            ...
            if len(highAct) > 0:
                ...
                for j in highAct:   #高活跃用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_highAct'],j))
                    self.conn.commit()   
            if len(modAct) > 0:
                for k in modAct:    #中度活跃用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_modAct'],k))
                    self.conn.commit()   
            if len(liteAct) > 0:
                for l in liteAct:   #轻度活跃用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_liteAct'],l))
                    self.conn.commit()   

    # 判断流失日期
    def leave_date(self,cursor,yz_id_list):
        yz_ids3 = []
        ...
        if len(yz_ids3) > 0:
            for i in yz_ids3:
                ...
                if nowTime-lastOrderTime >= 60*86400:
                    highLeave.append(i['yz_open_id'])
                elif nowTime-lastOrderTime >= 31*86400:
                    modLeave.append(i['yz_open_id'])
                elif nowTime-lastOrderTime >= 15*86400:
                    liteLeave.append(i['yz_open_id'])
            if len(highLeave)>0:
                for j in highLeave:   #高度流失用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_highLeave'],j))
                    self.conn.commit()   
            if len(modLeave) > 0:
                for k in modLeave:    #中度流失用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_modLeave'],k))
                    self.conn.commit()   
            if len(liteLeave) > 0:
                for l in liteLeave:   #轻度流失用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_liteLeave'],l))
                    self.conn.commit()   
    # 判断累计订单金额
    def sum_cost(self,cursor,yz_id_list):
        yz_ids4 = []

        cursor.execute('select yz_open_id,sum(cost) as sumCost from customer_order '
                       'where yz_open_id IN %s GROUP BY yz_open_id',((yz_id_list),))
        yz_ids4 = cursor.fetchall()
        ...
        if len(yz_ids4) > 0:
            ...
            if len(highWorth)>0:
                for j in highWorth:   #高价值用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_highWorth'],j))
                    self.conn.commit()   
            if len(modWorth) > 0:
                for k in modWorth:    #中价值用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_modWorth'],k))
                    self.conn.commit()   
            if len(liteWorth) > 0:
                for l in liteWorth:   #价值用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_liteWorth'],l))
                    self.conn.commit()   
            if len(diggable) > 0:
                for n in diggable:   #可挖掘用户写入
                    cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_diggable'],n))
                    self.conn.commit()   
    # 喜好组合查询
    def goods_like(self,cursor,yz_id_list):
        yz_ids5 = []
        ...
        if len(yz_ids5) > 0:
            for i in yz_ids5:
                cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)', (self.tag_list['tag_cp'], i['yz_open_id']))
                self.conn.commit()   
    # 只购买过一次的新用户
    def buy_once(self,cursor,yz_id_list):
        yz_ids6 = []
        ...
        yz_ids6 = cursor.fetchall()
        if len(yz_ids6) > 0:
            for i in yz_ids6:
                cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_buyOnce'], i['yz_open_id']))
                self.conn.commit()   
    # 巨划算用户——需要配置客户手机号数组
    def great_deal(self,cursor,yz_id_list):
        yz_ids7 = []
        # 数据表配置手机号
        ...
        yz_ids7 = cursor.fetchall()
        if len(yz_ids7)>0:
            for i in yz_ids7:
                cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',(self.tag_list['tag_bigWorth'], i['yz_open_id']))
                self.conn.commit()
    # 拓展名单用户
    def expand(self, cursor, yz_id_list):
        yz_ids9 = []
        cursor.execute(
            "select yz_open_id,tag_id from cus_expand where yz_open_id IN %s",((yz_id_list),))

        yz_ids9 = cursor.fetchall()
        if len(yz_ids9) > 0:
            for i in yz_ids9:
                cursor.execute('insert INTO customer_tag_affiliate(tag_id,yz_open_id)VALUES(%s,%s)',
                               (i['tag_id'], i['yz_open_id']))
                self.conn.commit()   
