...
def get_tc_config_price(cursor,goods_id,sku_detail_id,attr_config):
    ...
    for s_tc_config in s_tc_configs:
        ...
        if not cp_config_content['attr']:
            ...
            cursor.execute('select * from goods_sku_detail where goods_id = %s and sku_id = %s',(s_tc_config['dp_goods_id'], s_sku_id))
...
def get_cp_config_price(cursor,goods_id,sku_detail_id,attr_config):
    ...
    h_costs = 0
    handle_sku_price = []
    cursor.execute('select * from goods_cp_config where goods_id = %s and sku_detail_id = %s order by id asc',(goods_id, sku_detail_id))
    ...
    if cp_config_rows:
        ...
        if attr_config['auto_create_sale_price']==1:
            ...
            cursor.execute('select * from goods_sku_detail where goods_id = %s and id=%s', (goods_id,sku_detail_id))
            cp_sale_price = cursor.fetchone()
            if not cp_sale_price or cp_sale_price.get('sale_price',0)<=0:
                cp_sale_price['sale_price'] = 9999
            return {'cost': float('%.2f'%cost), 'sale_price': float('%.2f'%float(cp_sale_price['sale_price']))}
    return {'cost': float('%.2f' % 9999), 'sale_price': float('%.2f' % float(9999))}
...
class GoodsSyn:
    ...
    def execute_to(self, limit):
        ...
        if not is_run['switch_state']:
            ...
            # cursor.execute('select * from goods where id=2502 order by id asc')
            cursor.execute('select * from goods where is_delete = 0 and (version is null or version!=version) and auto_syn=1 and syn = 1 order by id desc limit %s',(limit))
            rows = cursor.fetchall()
            updates = []
            error_updates = []
            log_inserts = []
                        ...
                        if not row['id']:
                            ...
                            if desc:
                                params = {
                                    ...
                                    'item_type': 0,
                                    'cid': 3000000,
                                    'title': self.get_goods_title(row['name'],row['scientific_name'],row['nick_name'],row['adorn_name']),
                                    ...
                                    'self_pick': True if (2 & row['delivery_mode']) > 0 else False,
                                    'express': True if (1 & row['delivery_mode']) > 0 else False,
                                    'city_delivery': True if (4 & row['delivery_mode']) > 0 else False
                                }
                            else:
                                params = {
                                    'item_no': str(row['id']),
                            ...
                            response = self.api.invoke(api_name, '3.0.1', params)
                            if not (response.get('code',0) == 200):
                                syn_state = 2
                            else:
                                syn_state = 3
    ...
    def execute_delete(self,limit):
        ...
        if not is_run['switch_state']:
            ...
            cursor.execute('select id,id,updated_id from goods where id<>0 and is_delete = 1 and auto_syn=1 order by id asc limit %s',(limit))
            rows = cursor.fetchall()
            deletes = []
            log_inserts = []
            if rows:
                for row in rows:
                    ...
                    try:
                        ...
                        response = self.api.invoke(api_name, '3.0.1', {
                            'item_id': row['id']
                        })
                        if not (response['code'] == 200):
                            syn_state = 2
                ...
                if len(updates) > 0:
                    cursor.executemany("update goods set id = 0 where id = %s",updates)
                    conn.commit()
                if len(log_inserts) > 0:
                    cursor.executemany("insert into syn_log(response_content,created_time,type,syn_id,syn_state,api_name,updated_id) values(%s,%s,%s,%s,%s,%s,%s)",log_inserts)
                    conn.commit()
