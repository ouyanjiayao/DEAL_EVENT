...
class OrderGoodsDetail:
    ...
    def execute_to(self,type):
        ...
        sql = "select delivery_start_time as '配送时间',goods_name as '商品名称',spec_value as '规格',spec_val as '重量',type_val as 'PROCESS',sum(num) as '数量', if(weight=1,sum(total_num),sum(total_num)/500) as '总重量',if(weight=1,sum(weight),sum(weight)/500) as '计量单位',cate_name as '分类' into outfile  "+outfile+" character set gbk fields terminated by ',' optionally enclosed by '\"' lines terminated by '\n' from order_detail  where order_id>7957 and (delivery_start_time between "+start_time+" and "+end_time+") group by goods_name,spec_value,type_val ORDER BY id DESC"
        try:
            ...
            subject = now+'采购商品统计'
            content_text = '配送时间，商品名称，规格，重量，PROCESS，累计数量，累计总量，单位总量，分类'
