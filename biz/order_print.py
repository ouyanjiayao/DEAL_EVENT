...
class OrderPrintFirst:
    ...
    def executePrint(self, limit, order_ids=[]):
        ...
        if not is_run['switch_state']:
            ...
            getOrder = "select * from order where id in (%s) and order_state>1 and order_num is not null and (zt_print_state=0 or fk_print_state=0)  and delivery_start_time BETWEEN UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE)) AND UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE) + INTERVAL 1 DAY) order by id asc limit " % ','.join(['%s'] * len(order_ids))
            getOrders = getOrder + str(limit)
            cursor.execute(getOrders, order_ids)
                ...
                for tc_config_content in tc_config['tc_config']:
                    ...
                    for dp_config_content in tc_config_content['dp_config']:
                        ...
                        dp_item_contents.append(dp_item_content.format(
                            **{'dp_name': dp_config_content['dp_name'], 'count': '<FS2>'+str(int(dp_config_content['count'])*int(tc_config['count']))+'</FS2>',
