...
class OrderGoodsDetail:
    ...
    def execute_to(self,type):
        ...
        sql="select dp_name as '商品名称',sku_name as '规格',handle as 'PROCESS' ,weight as '重量',sum(weight*count) as '总重量',sum(count) as '数量',tag_name as '分类'into outfile  "+outfile+" character set gbk fields terminated by ',' optionally enclosed by '\"' lines terminated by '\n' from bill_dp_on where delivery_start_time between "+start_time+" and "+end_time+" group by dp_name,sku_name,handle ORDER BY id DESC"
        try:
