...
def get_tc_config_price(cursor,goods_id,sku_detail_id,attr_config):
    ...
    for s_tc_config in s_tc_configs:
        cp_config_content = json.loads(s_tc_conimfig['content'])
...
class GoodsSyn:
    ...
    def __init__(self):
        ...
        self.api = ApiClient()
        self.cursor = None
        self.app_tag = {self.default_config['youzan']['syn_id']: self.default_config['youzan']['grant_id'],self.default_config['qh']['syn_id']: self.default_config['qh']['grant_id']}
    ...
    def execute_to(self, limit):
        ...
        if not is_run['switch_state']:
            ...
            cursor.execute('select * from goods where is_delete = 0 and (version is null or version!=version) and auto_syn=1 and syn > 0 order by id asc limit %s',(limit))
            rows = cursor.fetchall()
            ...
            if rows:
                for row in rows:
                    ...
                    try:
                        ...
                        if not row['id']:
                            ...
                            for j in syn:
                                mirror = 'v1' if (j & 1) > 0 else 'qh'
                                response = self.api.invoke(api_name, version, params, None, mirror)
                                ...
                                if not row['updated_id']:
                                    row['updated_id'] = 0
                                log_inserts.append((response_content, time.time(), 1, row['id'], syn_state, api_name,row['updated_id'], self.app_tag[j]))
                        else:
                ...
                if len(log_inserts) > 0:
                    cursor.executemany("insert into syn_log(response_content,created_time,type,syn_id,syn_state,api_name,updated_id,offline_id) values(%s,%s,%s,%s,%s,%s,%s,%s)",log_inserts)
                cursor.close()
    ...
    def execute_delete(self,limit):
        ...
        if not is_run['switch_state']:
            ...
            cursor.execute('select id,id,updated_id from goods where is_delete = 1 and auto_syn=1 and syn>0 order by id asc limit %s',(limit))
            rows = cursor.fetchall()
            deletes = []
            log_inserts = []
            if rows:
