...
class RePurPrint:
    ...
    def execute(self, limit):
        ...
        if not is_run['switch_state']:
            ...
            cursor.execute('select * from print_queue where status = 0 and print_time BETWEEN UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE)) AND UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE) + INTERVAL 1 DAY) order by id limit %s',(limit))
            re_pur_rows = cursor.fetchall()
            if len(re_pur_rows) > 0:
                for row in re_pur_rows:
                    cursor.execute('select * from goods_print_queue where status = 0 and order_id = %s',(row['order_id']))
                    goods_queue = cursor.fetchall()
                    print_updates = []
                    try:
                        for i in self.devices:
                            if i['seller_tag']:
                                ...
                                print_content = self.generate_print_content(4, goods_queue, row)
                                print_device.printContent(print_content, row['order_id'])
                                state = 1
                    except Exception as e:
                        state = 2
                        order_control_logger.exception(e)
                    print_updates.append((state,time.time(),row['id']))
                    time.sleep(1)
                if len(print_updates)>0:
                    cursor.executemany("update goods_print_queue set status = %s, updated_time = %s where order_id = %s",print_updates)
    ...
    def generate_print_content(self,type,order_data,row):
        ...
        content = ''
        content += '         <FB><FS2>REPUR</FS2></FB>\r\n'
        content += '   <FB> </FB>\r\n\r\n'
        content += '<FS><FB>'        
        content += '         {title}</FB></FS>\r\n\r\n'
        if type != 3:
            content += '<FS><FB>ORDERNUM:{order_num} {user_name}</FB></FS>\r\n\r\n'
        ...
        if type == 3:
            ...
            content += '<center>扫码发货↓</center><QR>{url}</QR>\r\n\r\n'.format(
                **{'url': 'http://test.sto2c.com/youzan/api/logistics-confirm?id=' + str(row['id'])})
        if type == 4:
            content += '********************************\r\n'
            content += '<center>扫码完成↓</center><QR>{url}</QR>\r\n\r\n'.format(
                **{'url': str(row['id'])})
    ...
    def generate_order_data(self,seller_type,row,is_extra=False):
        ...
        if order_data:
            ...
            if seller_type:
                return self.in_order(order_data,seller_type)
            return order_data
        else:
            self.is_reserve = 0
    ...
    def in_order(self,order_data,seller_type):
        tag = str(seller_type)
        details = []
