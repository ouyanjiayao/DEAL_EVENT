...
class OrderMerPrint:
    ...
    def executeOrderPrint(self, limit):
        ...
        if not is_run['switch_state']:
            ...
            # cursor.execute('select * from order order by id asc')
                    ...
                    for j in self.devices:
                        if j['is_open']==1:
                            if j['seller_id']==1:
                                ...
                                # print_content = self.generate_refund_content(i)
                                print_content_order = self.generate_mer_refund_content(i,j['tag_id'])
                                if self.has_content:
                                    j['print_device'].printContent(print_content_order, i['order_id'])
                                    pass
                                state = 2
                except Exception as e:
                    state = 1
    ...
    # 商户退款单
    def generate_mer_refund_content(self,row,seller_tag=0):
        self.has_content = 0
        order_num = row['order_num']
        tid = row['tid']
        refund_time = time.strftime('%Y-%m-%d %H:%M', time.localtime(row['update_time']))
